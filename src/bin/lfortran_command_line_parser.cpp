...
namespace LCompilers::CommandLineInterface {
    ...
    auto LFortranCommandLineParser::parse() -> void {
        ...
        app.add_flag("--show-mlir", opts.show_mlir,
            "Show MLIR for the given file and exit; with `--backend=mlir-new-native` "
            "or `--backend=mlir-new-upstream` shows LLVM IR")->group(group_output_debugging_options);
        app.add_flag("--show-mlir-llvm-dialect", opts.show_mlir_llvm_dialect,
            "Show LLVM-dialect MLIR emitted by mlir-new backends and exit; "
            "implies mlir-new-native pipeline when set alone")->group(group_output_debugging_options);
        ...
        app.add_option("--backend", opts.arg_backend, "Select a backend (llvm, c, cpp, x86, wasm, fortran, mlir, mlir-new-native, mlir-new-upstream; mlir-new aliases mlir-new-native)")->capture_default_str()->group(group_backend_codegen_options);
