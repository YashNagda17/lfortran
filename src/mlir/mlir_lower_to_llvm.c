...
// alone.
//
// Entry points: MLIR_LowerToLLVMDialect (LLVM IR path) and
// MLIR_LowerToLLVMDialectForWasm (wasm pipeline; keeps scf.* in place).
...
// Lower `vector.print %x : T` to `printf` (libc): format global + variadic
// llvm.call with `var_callee_type` = `i32 (ptr, ...)`.
...
static bool lower_arith_constant(LowerState *st, MLIR_OpHandle op,
    ...
    if (ts.size == 5 && memcmp(ts.str, "index", 5) == 0) {
        ty = ty_i64(st->ctx);
...
// Lower `arith.index_cast` (e.g. i32 -> index) to `llvm.sext` into i64 for GEP.
...
static int try_lower_op(LowerState *st, MLIR_OpHandle op,
    ...
    if (name_eq(name, "memref.alloca"))
        ok = lower_memref_alloca(st, op, parent, pos);
    else if (name_eq(name, "memref.load"))
        ok = lower_memref_load(st, op, parent, pos);
    else if (name_eq(name, "memref.store"))
        ok = lower_memref_store(st, op, parent, pos);
    else if (name_eq(name, "func.func"))      ok = lower_func_func(st, op, parent, pos);
    ...
    else if (name_eq(name, "arith.index_cast")) ok = lower_arith_index_cast(st, op, parent, pos);
...
// Lower only `vector.print` to libc `printf` (hosted LFortran). Used before
// upstream's VectorToLLVM pass, which otherwise emits printI64/printNewline.
...
bool mlir_lower_vector_print_native(MLIR_Context *ctx, MLIR_OpHandle module) {
...
static bool run_lower(LowerState *st) {
    if (st->module == MLIR_INVALID_HANDLE) return false;
    if (MLIR_GetOpNumRegions(st->module) == 0) return false;
    MLIR_RegionHandle body_region = MLIR_GetOpRegion(st->module, 0);
    if (MLIR_GetRegionNumBlocks(body_region) == 0) return false;
    st->module_body = MLIR_GetRegionBlock(body_region, 0);
