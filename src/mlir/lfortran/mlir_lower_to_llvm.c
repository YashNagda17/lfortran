// LFortran override of upstream/mlir_lower_to_llvm.c: calls MLIR_LFortranTryLowerOp
// from mlir_lfortran_hooks.c before generic lowerings.
//
...
static int try_lower_op(LowerState *st, MLIR_OpHandle op,
    ...
    if (MLIR_LFortranTryLowerOp(&st->lfortran, op, parent, pos)) {
...
bool MLIR_LowerToLLVMDialect(MLIR_Context *ctx, MLIR_OpHandle module) {
    ...
    MLIR_LFortranLowerStateInit(&st.lfortran, ctx, module);
