// LFortran LLVM-dialect lowering. Based on upstream/mlir_lower_to_llvm.c
// with hook macro injection points; the LFortran-specific hook macros are
// defined directly in this file. When upstream changes, refresh the body
// from upstream and re-apply the MLIR_LOWER_* hook block and sites.
...
// LFortran-specific lowering customization is defined directly here.
#define MLIR_LOWER_STATE_EXTRA \
    MLIR_LFortranLowerState lfortran;
#define MLIR_LOWER_STATE_INIT(st) \
    MLIR_LFortranLowerStateInit(&(st)->lfortran, (st)->ctx, (st)->module)
#define MLIR_LOWER_TRY_CUSTOM_OP(st, op, parent, pos) \
    MLIR_LFortranTryLowerOp(&(st)->lfortran, op, parent, pos)
#define MLIR_LOWER_PROVIDE_VECTOR_PRINT 0
...
#if MLIR_LOWER_PROVIDE_VECTOR_PRINT
...
#endif // MLIR_LOWER_PROVIDE_VECTOR_PRINT
...
static int try_lower_op(LowerState *st, MLIR_OpHandle op,
                        MLIR_BlockHandle parent, size_t pos) {
    if (MLIR_LOWER_TRY_CUSTOM_OP(st, op, parent, pos))
