// LFortran override of upstream/mlir_api_impl_upstream.cpp: ASR dialect region
// helpers, null-region CreateOp handling, and vector.print prepass via hooks.
//
...
extern "C" MLIR_OpHandle MLIR_CreateOpWithSuccessors(
    ...
    for (size_t i = 0; i < n_regions; i++) {
        if (regions && regions[i] != MLIR_INVALID_HANDLE) {
            state.addRegion(
                std::unique_ptr<mlir::Region>(F<mlir::Region>(regions[i])));
        } else {
            auto empty = std::make_unique<mlir::Region>();
            empty->push_back(new mlir::Block());
            state.addRegion(std::move(empty));
        }
    }
...
extern "C" MLIR_RegionHandle MLIR_EnsureOpRegion(MLIR_Context *, MLIR_OpHandle oh,
                                                    size_t idx) {
    auto *op = F<mlir::Operation>(oh);
    if (idx >= op->getNumRegions()) {
        return MLIR_INVALID_HANDLE;
    }
    return H(&op->getRegion(idx));
}
extern "C" MLIR_BlockHandle MLIR_EnsureRegionEntryBlock(MLIR_Context *,
                                                         MLIR_RegionHandle rh) {
    auto *region = F<mlir::Region>(rh);
    if (region->empty()) {
        region->push_back(new mlir::Block());
    }
    return H(&region->front());
}
...
static bool run_vector_print_prepass(mlir::ModuleOp module,
                                     MLIR_Context *ctx,
                                     MLIR_OpHandle module_h,
                                     const char *where) {
    bool has_vector_print = false;
    module.walk([&](mlir::Operation *o) {
        if (mlir::isa<mlir::vector::PrintOp>(o)) {
            has_vector_print = true;
            return mlir::WalkResult::interrupt();
        }
        return mlir::WalkResult::advance();
    });
    if (!has_vector_print) return true;
    MLIR_LFortranLowerState lf_st;
    MLIR_LFortranLowerStateInit(&lf_st, ctx, module_h);
    if (!mlir_lower_vector_print_native(&lf_st)) {
        std::fprintf(stderr,
                     "%s: vector.print lowering failed\n", where);
        return false;
    }
    return true;
}
...
extern "C" bool MLIR_LowerToLLVMDialectUpstream(MLIR_Context *ctx,
    ...
    if (!run_vector_print_prepass(module, ctx, module_h,
...
extern "C" bool MLIR_LowerToLLVMDialectForWasmUpstream(MLIR_Context *ctx,
    ...
    opByNameAttrs().clear();
    if (!run_vector_print_prepass(module, ctx, module_h,
                                  "MLIR_LowerToLLVMDialectForWasmUpstream")) {
        return false;
    }
    bool has_vector_op = false;
    module.walk([&](mlir::Operation *o) {
...
extern "C" bool MLIR_LowerToLLVMDialectForWasmUpstream(MLIR_Context *ctx,
    ...
    (void)module_h;
    std::fprintf(stderr,
        "MLIR_LowerToLLVMDialectForWasmUpstream: WebAssembly lowering is disabled "
        "(reconfigure with -DLFORTRAN_MLIR_WASM_LOWERING=yes)\n");
    return false;
}
...
extern "C" string MLIR_TranslateModuleToWasmUpstream(MLIR_Context *ctx,
    ...
    (void)module_h;
    std::fprintf(stderr,
        "MLIR_TranslateModuleToWasmUpstream: WebAssembly lowering is disabled "
        "(reconfigure with -DLFORTRAN_MLIR_WASM_LOWERING=yes)\n");
    return mkRefString(llvm::StringRef());
}
