// LFortran native LLVM IR translation. Based on upstream/
// mlir_translate_to_llvm_ir.c with hook macro injection points; the
// LFortran-specific hook macros are defined directly in this file. When
// upstream changes, refresh the body from upstream and re-apply the
// MLIR_LLVM_IR_* hook block and sites.
...
// LFortran-specific LLVM IR translation customization is defined directly here.
typedef MLIR_LFortranBuf Buf;
#define MLIR_LLVM_IR_PRINT_INDEX_TYPE(out, ctx, ty) \
    do { \
        if (!MLIR_LFortranPrintGepIndexType((out), (ctx), (ty), print_type)) \
            print_type((out), (ctx), (ty)); \
    } while (0)
#define MLIR_LLVM_IR_SHOULD_EMIT_BLOCK_LABEL(block_index) \
    MLIR_LFortranShouldEmitBlockLabel(block_index)
        ...
        if (ria != MLIR_INVALID_HANDLE) {
            ...
            while (i < s.size) {
                ...
                if (v == (long long)(int32_t)0x80000000) {
                    ...
                    buf_cstr(out, ", ");
                    MLIR_LLVM_IR_PRINT_INDEX_TYPE(out, F->ctx, MLIR_GetValueType(dv));
                    buf_putc(out, ' ');
                    emit_use(F, dv);
                } else {
...
static void emit_function(MLIR_Context *ctx, Buf *out, MLIR_OpHandle fn) {
    ...
    for (size_t bi = 0; bi < nb; bi++) {
        MLIR_BlockHandle blk = MLIR_GetRegionBlock(body, bi);
        if (MLIR_LLVM_IR_SHOULD_EMIT_BLOCK_LABEL(bi)) {
            buf_printf(out, "%s:\n", bname(&F, blk));
        }
