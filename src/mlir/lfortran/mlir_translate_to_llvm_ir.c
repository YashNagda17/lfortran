// LFortran override of upstream/mlir_translate_to_llvm_ir.c: uses
// mlir_lfortran_hooks.c for GEP index types and block labels.
//
...
static void print_gep_index_type(Buf *out, MLIR_Context *ctx, MLIR_TypeHandle ty) {
    if (!MLIR_LFortranPrintGepIndexType(
            out, ctx, ty, (MLIR_LFortranPrintTypeFn)print_type)) {
        ...
        if (ria != MLIR_INVALID_HANDLE) {
            ...
            while (i < s.size) {
                ...
                if (v == (long long)(int32_t)0x80000000) {
                    ...
                    buf_cstr(out, ", ");
                    print_gep_index_type(out, F->ctx, MLIR_GetValueType(dv));
                    buf_putc(out, ' ');
                    emit_use(F, dv);
                } else {
...
static void emit_function(MLIR_Context *ctx, Buf *out, MLIR_OpHandle fn) {
    ...
    for (size_t bi = 0; bi < nb; bi++) {
        MLIR_BlockHandle blk = MLIR_GetRegionBlock(body, bi);
        if (MLIR_LFortranShouldEmitBlockLabel(bi)) {
            buf_printf(out, "%s:\n", bname(&F, blk));
        }
