...
def field_kind(asdl_type: str, seq: bool, opt: bool) -> str:
    if asdl_type in ("expr", "stmt", "symbol", "ttype", "node"):
        ...
        base = "VOID"
    elif asdl_type in SIMPLE_SUM_NAMES or asdl_type.endswith("Type"):
        base = asdl_type.upper()
    else:
        base = "OP"
    if seq:
...
def gen_api_generated_h(ops: list[dict]) -> str:
    ...
    for op in ops:
        ...
        for f in stored:
            fname = f.name or f.type
            if f.type in ("expr", "stmt", "symbol", "ttype", "node"):
                params.append(f"MLIR_ValueHandle {fname}")
            elif f.type == "identifier" or f.type == "string":
        ...
        lines.append("        " + ",\n        ".join(params) + ") {")
        if stored:
            lines.append(f"    ASR_DialectField fields[{len(stored)}];")
            for i, f in enumerate(stored):
                fname = f.name or f.type
                fk = field_kind(f.type, f.seq, f.opt)
                if f.type in ("expr", "stmt", "symbol", "ttype", "node"):
                    lines.append(f"    fields[{i}] = (ASR_DialectField){{"
                                 f"{fk}, \"{fname}\", .value.value = {fname}}};")
                elif f.type in ("identifier", "string"):
                    lines.append(f"    fields[{i}] = (ASR_DialectField){{"
                                 f"{fk}, \"{fname}\", .value.str = {fname}}};")
                elif f.type == "int":
                    lines.append(f"    fields[{i}] = (ASR_DialectField){{"
                                 f"{fk}, \"{fname}\", .value.i64 = {fname}}};")
                elif f.type == "bool":
                    lines.append(f"    fields[{i}] = (ASR_DialectField){{"
                                 f"{fk}, \"{fname}\", .value.b = {fname}}};")
                elif f.type == "float":
                    lines.append(f"    fields[{i}] = (ASR_DialectField){{"
                                 f"{fk}, \"{fname}\", .value.f64 = {fname}}};")
                else:
                    lines.append(f"    fields[{i}] = (ASR_DialectField){{"
                                 f"{fk}, \"{fname}\", .value.op = {fname}}};")
        else:
            lines.append("    (void)ctx; (void)loc;")
        n = len(stored)
        lines.append(f"    return ASR_DialectCreateOp(ctx, {op['kind']}, loc,"
                     f" fields, {n});")
        lines.append("}")
        lines.append("")
    return "\n".join(lines)
...
def cpp_emit_field(field: asdl.Field) -> str:
    ...
    if asdl_type == "identifier":
        ...
        return f"        string {fname} = str_from_cstr(x.{member});"
    if asdl_type == "string":
        if field.opt:
            return (f"        string {fname} = str_lit(\"\");\n"
                    f"        if (x.{member}) {{ {fname} = str_from_cstr(x.{member}); }}")
        return f"        string {fname} = str_from_cstr(x.{member});"
    if asdl_type == "int":
...
def gen_lowering_dispatch_inc(ops: list[dict]) -> str:
    ...
    lines.append("}")
    lines.append("")
    lines.append("// Generated lowering handler declarations")
    for op in ops:
        if op["name"] in LOWERED_OPS:
            lines.append(f"bool ASR_Lower{op['name']}(ASR_LoweringContext *ctx, MLIR_OpHandle op);")
    lines.append("")
    return "\n".join(lines)
...
def main() -> int:
    ...
    if args.check:
        return 0 if check_outputs(ops, asdl_hash, args.out_mlir_dir, args.out_lfortran_dir) else 1

    write_outputs(ops, asdl_hash, args.out_mlir_dir, args.out_lfortran_dir)
    return 0
