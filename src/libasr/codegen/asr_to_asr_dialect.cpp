...
#define ASR_DIALECT_API_GENERATED_INCLUDE_CREATORS
...
MLIR_OpHandle *ASRToAsrDialectVisitor::emit_stmt_op_array(
        ASR::stmt_t **stmts, size_t n, size_t *n_out) {
    if (n_out) {
        *n_out = 0;
    }
    if (n == 0 || !stmts) {
...
MLIR_OpHandle *ASRToAsrDialectVisitor::copy_op_handles(
        const std::vector<MLIR_OpHandle> &ops, size_t *n_out) {
    size_t n = ops.size();
    if (n_out) {
        *n_out = n;
    }
...
MLIR_RegionHandle ASRToAsrDialectVisitor::emit_empty_region() {
    MLIR_RegionHandle region = MLIR_CreateRegion(&ctx);
    MLIR_BlockHandle block = MLIR_CreateBlock(&ctx);
    MLIR_AppendRegionBlock(&ctx, region, block);
    return region;
}

MLIR_RegionHandle ASRToAsrDialectVisitor::emit_op_vector_region(
        const std::vector<MLIR_OpHandle> &ops) {
    MLIR_RegionHandle region = MLIR_CreateRegion(&ctx);
    MLIR_BlockHandle block = MLIR_CreateBlock(&ctx);
    MLIR_AppendRegionBlock(&ctx, region, block);
    for (MLIR_OpHandle op : ops) {
        if (op != MLIR_INVALID_HANDLE) {
            MLIR_AppendBlockOp(&ctx, block, op);
        }
    }
    return region;
}

MLIR_RegionHandle ASRToAsrDialectVisitor::emit_stmt_region(
        ASR::stmt_t **stmts, size_t n) {
    if (n == 0 || !stmts) {
        return emit_empty_region();
    }
    ...
    suppress_module_append = false;
    return emit_op_vector_region(body_ops);
}
...
void ASRToAsrDialectVisitor::append_current_stmt(MLIR_OpHandle op) {
    if (op != MLIR_INVALID_HANDLE && current_stmt_loc.first != 0) {
        ASR_AttachFortranLocAttrs(&ctx, op, current_stmt_loc.first,
            current_stmt_loc.last);
    }
...
void ASRToAsrDialectVisitor::visit_DoLoop(const ASR::DoLoop_t &x) {
    ...
    MLIR_OpHandle *body = emit_stmt_op_array(x.m_body, n_body, &n_body);
    last_value = ASR_CreateDoLoopOp(&ctx, default_loc(), asr_cstr(x.m_name),
        head, body, n_body, nullptr, 0);
    append_current_stmt(last_value);
}

void ASRToAsrDialectVisitor::visit_If(const ASR::If_t &x) {
    MLIR_OpHandle test = emit_expr(*x.m_test);
    size_t n_body = x.n_body;
    MLIR_OpHandle *body = emit_stmt_op_array(x.m_body, n_body, &n_body);
    size_t n_orelse = x.n_orelse;
    MLIR_OpHandle *orelse = emit_stmt_op_array(x.m_orelse, n_orelse, &n_orelse);
    last_value = ASR_CreateIfOp(&ctx, default_loc(), asr_cstr(x.m_name),
        test, body, n_body, orelse, n_orelse);
    append_current_stmt(last_value);
}
...
void ASRToAsrDialectVisitor::visit_WhileLoop(const ASR::WhileLoop_t &x) {
    ...
    size_t n_orelse = x.n_orelse;
    MLIR_OpHandle *orelse = emit_stmt_op_array(x.m_orelse, n_orelse, &n_orelse);
    last_value = ASR_CreateWhileLoopOp(&ctx, default_loc(), asr_cstr(x.m_name),
        test, body, n_body, orelse, n_orelse);
    append_current_stmt(last_value);
}
...
void ASRToAsrDialectVisitor::visit_Select(const ASR::Select_t &x) {
    ...
    size_t n_default_ = x.n_default;
        MLIR_OpHandle *default_ = emit_stmt_op_array(x.m_default, n_default_, &n_default_);
...
void ASRToAsrDialectVisitor::visit_Var(const ASR::Var_t &x) {
    ...
    MLIR_TypeHandle type = convert_type(*ASRUtils::symbol_type(x.m_v));
    MLIR_LocationHandle op_loc = loc(x.base.base.loc);
    ...
    last_value = ASR_DialectCreateOp(&ctx, ASR_DIALECT_OP_EXPR_VAR, op_loc, fields, 2);
}
...
void ASRToAsrDialectVisitor::visit_Function(const ASR::Function_t &x) {
    ...
    size_t n_symtab = scope_symtab.size();
    MLIR_OpHandle *symtab = copy_op_handles(scope_symtab, &n_symtab);
    size_t n_metadata = scope_metadata.size();
    MLIR_OpHandle *metadata = copy_op_handles(scope_metadata, &n_metadata);
    size_t n_body = scope_body.size();
    MLIR_OpHandle *body = copy_op_handles(scope_body, &n_body);

    ASR_DialectField fields[13] = {};
    size_t nf = 0;
    fields[nf].kind = ASR_FIELD_OP;
    fields[nf].name = "symtab";
    fields[nf].value.op_seq.items = symtab;
    fields[nf].value.op_seq.n_items = n_symtab;
    ++nf;
    fields[nf].kind = ASR_FIELD_OP;
    fields[nf].name = "metadata";
    fields[nf].value.op_seq.items = metadata;
    fields[nf].value.op_seq.n_items = n_metadata;
    ++nf;
    fields[nf].kind = ASR_FIELD_IDENTIFIER;
    fields[nf].name = "name";
    fields[nf].value.str = asr_cstr(x.m_name);
    ++nf;
    fields[nf].kind = ASR_FIELD_TTYPE;
    fields[nf].name = "function_signature";
    fields[nf].value.type = fn_sig;
    ++nf;
    fields[nf].kind = ASR_FIELD_IDENTIFIER_SEQ;
    fields[nf].name = "dependencies";
    fields[nf].value.str = deps;
    ++nf;
    fields[nf].kind = ASR_FIELD_EXPR_SEQ;
    fields[nf].name = "args";
    fields[nf].value.op_seq.items = nullptr;
    fields[nf].value.op_seq.n_items = 0;
    ++nf;
    fields[nf].kind = ASR_FIELD_STMT_SEQ;
    fields[nf].name = "body";
    fields[nf].value.op_seq.items = body;
    fields[nf].value.op_seq.n_items = n_body;
    ++nf;
    fields[nf].kind = ASR_FIELD_I64;
    fields[nf].name = "access";
    fields[nf].value.i64 = (int64_t)x.m_access;
    ++nf;
    fields[nf].kind = ASR_FIELD_BOOL;
    fields[nf].name = "deterministic";
    fields[nf].value.b = x.m_deterministic;
    ++nf;
    fields[nf].kind = ASR_FIELD_BOOL;
    fields[nf].name = "side_effect_free";
    fields[nf].value.b = x.m_side_effect_free;
    ++nf;
    fields[nf].kind = ASR_FIELD_STRING_OPT;
    fields[nf].name = "module_file";
    fields[nf].value.str = x.m_module_file ? asr_cstr(x.m_module_file) : str_lit("");
    ++nf;

    MLIR_OpHandle fn_op = ASR_DialectCreateOp(&ctx, ASR_DIALECT_OP_SYMBOL_FUNCTION,
        default_loc(), fields, nf);
...
void ASRToAsrDialectVisitor::visit_Program(const ASR::Program_t &x) {
    ...
    program_op = ASR_DialectCreateOp(&ctx, ASR_DIALECT_OP_SYMBOL_PROGRAM,
        default_loc(), fields, nf);
...
void ASRToAsrDialectVisitor::visit_TranslationUnit(const ASR::TranslationUnit_t &x) {
    ...
    fields[0].kind = ASR_FIELD_NODE_SEQ;
    fields[0].name = "items";
    fields[0].value.op_seq.items = items;
    fields[0].value.op_seq.n_items = 1;
    last_value = ASR_DialectCreateOp(&ctx, ASR_DIALECT_OP_UNIT_TRANSLATIONUNIT,
