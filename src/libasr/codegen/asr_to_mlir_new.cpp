// Must be set before any #include of mlir_new_backend.h (asr_to_mlir_new.h
// includes it first; the header guard would skip a later define otherwise).
#define MLIR_NEW_BACKEND_INCLUDE_API
...
struct LlvmEmitter {
    ...
    const MlirNewApi *api = nullptr;

    explicit LlvmEmitter(const MlirNewApi *api_) : api(api_) {}
    ...
    MLIR_OpHandle emit_simple_op(MLIR_OpType ty, string opname,
        ...
        return api->CreateOp(&ctx, ty, opname, attrs, n_attrs, result_types,
    ...
    void emit_branch(MLIR_BlockHandle target) {
        ...
        size_t snums[1] = {0};
        MLIR_OpHandle op = api->CreateOpWithSuccessors(
    ...
    void emit_module_skeleton() {
        ...
        api->SetArenaAllocator(&ctx, arena);
        loc = api->CreateLocationUnknown(&ctx, str_lit("lfortran"));
        i32_ty = api->CreateTypeInteger(&ctx, 32, true);
        i64_ty = api->CreateTypeInteger(&ctx, 64, false);
        i1_ty = api->CreateTypeInteger(&ctx, 1, false);
        ptr_ty = api->CreateTypeLLVMPointer(&ctx);
        void_ty = api->CreateTypeLLVMVoid(&ctx);

        MLIR_RegionHandle mr = api->CreateRegion(&ctx);
        module_block = api->CreateBlock(&ctx);
        api->AppendRegionBlock(&ctx, mr, module_block);
        MLIR_RegionHandle mregs[1] = {mr};
        module_op = api->CreateOp(&ctx, OP_TYPE_MODULE, str_lit("module"),
    ...
    MLIR_ValueHandle emit_const_i32(int64_t value) {
        MLIR_ValueHandle res = api->CreateValueOpResult(
        ...
        MLIR_AttributeHandle val =
            api->CreateAttributeInteger(&ctx, str_lit("value"), value, i32_ty);
    ...
    MLIR_ValueHandle emit_const_i64(int64_t value) {
        ...
        MLIR_ValueHandle rs[1] = {res};
        MLIR_AttributeHandle val =
            api->CreateAttributeInteger(&ctx, str_lit("value"), value, i64_ty);
        MLIR_AttributeHandle as[1] = {val};
        MLIR_OpHandle op = emit_simple_op(OP_TYPE_LLVM_MLIR_CONSTANT,
            str_lit("llvm.mlir.constant"), as, 1, rts, 1, rs, 1, nullptr, 0,
            nullptr, 0);
    ...
    MLIR_ValueHandle emit_alloca(MLIR_TypeHandle elem_ty) {
        MLIR_ValueHandle res = api->CreateValueOpResult(&ctx,
        ...
        MLIR_AttributeHandle elem_attr =
            api->CreateAttributeType(&ctx, str_lit("elem_type"), elem_ty);
    ...
    MLIR_ValueHandle emit_gep(MLIR_ValueHandle base,
        ...
        MLIR_AttributeHandle raw_attr = api->CreateAttributeDenseI32Array(
            &ctx, str_lit("rawConstantIndices"), raw_idx, n_raw);
        MLIR_AttributeHandle elem_attr =
            api->CreateAttributeType(&ctx, str_lit("elem_type"), source_elem_ty);
    ...
    MLIR_ValueHandle emit_const_i1(bool value) {
        ...
        MLIR_TypeHandle rts[1] = {i1_ty};
        MLIR_ValueHandle rs[1] = {res};
        MLIR_AttributeHandle val = api->CreateAttributeInteger(
    ...
    MLIR_ValueHandle emit_icmp_i32(int64_t predicate, MLIR_ValueHandle a,
        ...
        MLIR_ValueHandle rs[1] = {res};
        MLIR_ValueHandle ops[2] = {a, b};
        MLIR_AttributeHandle pred = api->CreateAttributeInteger(&ctx,
    ...
    MLIR_OpHandle emit_llvm_func(string sym, MLIR_TypeHandle fn_ty,
        ...
        MLIR_AttributeHandle sym_name =
            api->CreateAttributeString(&ctx, str_lit("sym_name"), sym);
        MLIR_AttributeHandle fn_ty_attr =
            api->CreateAttributeType(&ctx, str_lit("function_type"), fn_ty);
        ...
        MLIR_OpHandle fn = api->CreateOp(&ctx, OP_TYPE_UNREGISTERED,
    ...
    void emit_module_runtime_decls() {
        if (!printf_declared) {
            MLIR_TypeHandle fn_ty = api->CreateTypeLLVMFunction(&ctx, i32_ty,
                &ptr_ty, 1, true);
            MLIR_RegionHandle empty_r = api->CreateRegion(&ctx);
            append_module(emit_llvm_func(str_lit("printf"), fn_ty, empty_r));
            printf_declared = true;
        }
        if (!fmt_global_added) {
            MLIR_OpHandle fmt_global = api->CreateLLVMGlobalString(&ctx,
    ...
    void emit_printf_i32(MLIR_ValueHandle val) {
        emit_module_runtime_decls();
        MLIR_ValueHandle fmt_ptr = api->CreateValueOpResult(&ctx,
            MLIR_INVALID_HANDLE, 0, ptr_ty,
            arena_ssa_name(arena, ssa_counter++), loc);
        MLIR_TypeHandle rts[1] = {ptr_ty};
        MLIR_ValueHandle rs[1] = {fmt_ptr};
        MLIR_AttributeHandle global_name = api->CreateAttributeSymbolRef(&ctx,
        ...
        MLIR_TypeHandle call_ty = api->CreateTypeLLVMFunction(&ctx, i32_ty,
            &ptr_ty, 1, true);
        MLIR_AttributeHandle callee =
            api->CreateAttributeSymbolRef(&ctx, str_lit("callee"), str_lit("printf"));
        MLIR_AttributeHandle var_ty = api->CreateAttributeType(&ctx,
        ...
        MLIR_ValueHandle call_res = api->CreateValueOpResult(&ctx,
            MLIR_INVALID_HANDLE, 0, i32_ty,
            arena_ssa_name(arena, ssa_counter++), loc);
    ...
    void emit_variable_storage(const ASR::Variable_t &x) {
        ...
        if (is_fixed_integer_array(x.m_type, array_len)) {
            MLIR_TypeHandle arr_ty = api->CreateTypeLLVMArray(
    ...
    void visit_Program(const ASR::Program_t &x) {
        ...
        fn_body_r = api->CreateRegion(&ctx);
        cur_block = api->CreateBlock(&ctx);
        api->AppendRegionBlock(&ctx, fn_body_r, cur_block);
        block_terminated = false;
...
Result<std::unique_ptr<MLIRModule>> asr_to_mlir_new(Allocator &al,
    ...
    const MlirNewApi *api = mlir_new_api_for(backend);
    LlvmEmitter emit(api);
    try {
    ...
    std::string llvm_dialect_mlir = copy_mlir_string(
        api->PrintModule(&emit.ctx, emit.module_op));

    string llvm_s = api->TranslateToLLVMIR(&emit.ctx, emit.module_op);
    std::string llvm_ir = copy_mlir_string(llvm_s);
    if (llvm_ir.empty()) {
        diagnostics.diagnostics.push_back(diag::Diagnostic(
            "asr_to_mlir_new: MLIR translate to LLVM IR returned empty output",
            diag::Level::Error, diag::Stage::CodeGen));
        arena_destroy(emit.arena);
        return Error();
    }
    ...
    arena_destroy(emit.arena);
    return Result<std::unique_ptr<MLIRModule>>(std::make_unique<MLIRModule>(
        std::move(llvm_ir), std::move(llvm_dialect_mlir)));
}
